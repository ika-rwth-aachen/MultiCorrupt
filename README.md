...
$$RA_{c,s} = \frac{{M}_{c,s}}{{M}_{clean}}, RA_c = \frac{1}{3} \sum_{s=1}^{3} RA_{c,s}$$

$$mRA = \frac{1}{N} \sum_{c=1}^{N} RA_c$$

where  ${M}_{c,s}$  represents metric for the $c$ types of corruption at the $s$-the severity level, $N$ is the total number of corruption types considered in our benchmark, and ${M}_{clean}$ is performance on the "clean" nuScenes dataset.
...
$$RRA_{c} = \frac{\sum\limits_{i=1}^{3} ({M}_{c, s})}{\sum\limits_{i=1}^{3} ({M}_{baseline, c, s})} - 1,$$
